...
function! <SID>polymehr_sudo_write(file)
    if len(a:file)
...
"      empty)
function! <SID>polymehr_focus_floating(num)
    if len(a:num)
        let l:num = a:num
    else
        let l:num = 1
...
command! -nargs=? -complete=file Swrite call <SID>polymehr_sudo_write('<args>')

command! -nargs=? -complete=file FocusFloating call <SID>polymehr_focus_floating('<args>')
nnoremap <silent> <leader>F :<C-U>call <SID>polymehr_focus_floating(v:count1)<CR>
