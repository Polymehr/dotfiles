...
# shellcheck source=/home/polymehr/.device_specific/default_sink.sh
...
if [ "${1:?"First should be non-empty!"}" = 'select' ]; then
    if [ "$2" == 'unset' ] && [ "$3" == 'active' ]; then
        [ -f '/tmp/polymehr/volume/active' ] && rm '/tmp/polymehr/volume/active'
        exit
    fi
    ...
    if [ "$2" == 'set' ] && [ "$3" == 'active' ]; then
        [ -d '/tmp/polymehr/volume/' ] || mkdir -p '/tmp/polymehr/volume/'
        printf '%s\n' "${SINKS[0]}" > '/tmp/polymehr/volume/active'
        exit
    fi
    shift 1
elif [ -f '/tmp/polymehr/volume/active' ]; then
    SINKS=( "$(cat '/tmp/polymehr/volume/active')" )
fi
...
if [ -n "$INS" ] || [ $USE_MUTE ]; then
    if [ -n "$INS" ]; then
        notify-send -a "polymehr:volume" -u low -h "int:value:$NEW" 'Volume ' "%$MUTED"
    else
        notify-send -a "polymehr:volume:mute_action" -u low 'Volume ' "$MUTED"
    fi
fi
